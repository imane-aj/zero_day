...
import { Messages, deleteMsg } from '../../Redux/MessageSlice';

const MessagesList = ({dispatch,searchQuery,}) => {
  ...
  useEffect(() => {
    dispatch(Messages(`${url}messages?page=1`));
  }, [dispatch]);

  const handlePagination = (url) => {
    console.log('Pagination URL:', url);
    dispatch(Messages(url));
  };
  ...
  const handelDeleteBO = (id) =>{
      Swal.fire({
        ...
        if (result.isConfirmed) {
            dispatch(deleteMsg(id)).then((res) => {
              if (res.type === 'message/deleteMsg/fulfilled') {
  ...
  useEffect(() => {
    if (searchResults.data && searchQuery) {
      ...
      setDataToDisplay(messages.data);
    }
  }, [searchResults, messages.data, searchQuery]);
  return (
   <>
      ...
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          {dataToDisplay?.map((val,idx)=>(
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">
                {val.subject == 'null' ? ' - ' : val.subject}
                </p>
              </td>
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
              
                  <Link className="hover:text-primary" to = {`view_message/${val.id}`} >
                    <svg
    ...
    <div className='mt-10 flex flex-row justify-between'>
      {messages?.next_page_url && (
        ...
        onClick={() => handlePagination(messages.next_page_url)}>
          Next <i class="fa-solid fa-chevron-right"></i>
        </button>
      )}
      {messages?.prev_page_url && (
        <button className='text-black hover:bg-bodydark1 transition duration-700 dark:text-white px-6 py-3 bg-white rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark' 
        onClick={() => handlePagination(messages.prev_page_url)}>
