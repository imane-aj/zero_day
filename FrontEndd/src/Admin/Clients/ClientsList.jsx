...
import { Clients, deleteClient } from '../../Redux/ClientSlice';
import { url } from '../../Redux/Utils';

export default function ClientsList({dispatch, searchQuery}) {
    const clients = useSelector((state) => state.client.data)
    const isLoading = useSelector((state) => state.client.isLoading)
    const searchResults = useSelector((state) => state.client.search)
    const [dataToDisplay, setDataToDisplay] = useState([]); // Initially set to all data
    useEffect(() => {
      dispatch(Clients(`${url}clients?page=1`));
    }, [dispatch]);
  
    const handlePagination = (url) => {
      dispatch(Clients(url));
    };
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
              dispatch(deleteClient(id)).then((res) => {
                if (res.type === 'client/deleteClient/fulfilled') {
    ...
    useEffect(() => {
      if (searchResults.data && searchQuery) {
        ...
        setDataToDisplay(clients.data);
      }
    }, [searchResults, clients.data, searchQuery]);
  return (
    <Fragment>
            ...
            <table className="w-full table-auto">
                ...
                {dataToDisplay?.map((val,idx)=>(
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <a href={`mailto:${val?.email}`} className="text-blue dark:text-white">
                            {val?.email}
                        </a>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    ...
                    <div className="flex items-center space-x-3.5">
                        ...
                        <button className="hover:text-primary text-danger m-auto" onClick={()=>handelDelete(val.id)}>
                        <svg
        ...
        <div className='mt-10 flex flex-row justify-between'>
            ...
            onClick={() => handlePagination(clients.next_page_url)}>
                Next <i class="fa-solid fa-chevron-right"></i>
            </button>
            )}
            {clients?.prev_page_url && (
            <button className='text-black hover:bg-bodydark1 transition duration-700 dark:text-white px-6 py-3 bg-white rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark'
            onClick={() => handlePagination(clients.prev_page_url)}>
