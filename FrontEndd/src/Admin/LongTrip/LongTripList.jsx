...
const LongTripList = ({dispatch,searchQuery,}) => {
    ...
    console.log(id)
      Swal.fire({
        ...
        if (result.isConfirmed) {
          dispatch(deleteTrip(id)).then((res) => {
            console.log('res.data', res)
            if (res.type === 'trip/deleteTrip/fulfilled') {
  ...
  }, [searchResults, trip, searchQuery]);
  return (
   <>
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {dataToDisplay?.map((val,idx)=>(
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white m-auto">{val.price} €</p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <Link className="hover:text-primary" to = {`edit_voyage_long_duree/${val.id}`}>
