...
const ClientCompte = () => {
  ...
  useEffect(() => {
    dispatch(Users(`${url}users?page=1`));
  }, [dispatch]);
  ...
  useEffect(() => {
    if (searchResults?.data && searchQuery) {
      ...
      setDataToDisplay(users?.data?.data);
    }
  }, [searchResults, users?.data, searchQuery]);
      ...
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          {dataToDisplay?.map((val,idx)=>(
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">{val.phone ? val.phone : '-'} </p>
              </td>
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
              
                 
                  <button className="hover:text-primary text-danger m-auto" onClick={()=>handelDeleteBO(val.id)}>
                    <svg
