...
import { Services, storePrice, updateService } from '../../../Redux/RequestSlice';
const ServiceList = ({dispatch,searchQuery,}) => {
  const service = useSelector((state) => state.req.data)
  const isLoading = useSelector((state) => state.req.isLoading)
  const searchResults = useSelector((state) => state.req.search)
  const [dataToDisplay, setDataToDisplay] = useState([]); // Initially set to all data
  const [inputStates, setInputStates] = useState({});
  const [price, setPrice] = useState(0);
 
  useEffect(() => {
    dispatch(Services(`${url}service_request?page=1`));
  }, [dispatch]);

  const handlePagination = (url) => {
    console.log('Pagination URL:', url);
    dispatch(Services(url));
  };

  const handleStatusChange = (e ,id) => {
    e.preventDefault();
    const status = 'Traité';
    const cpage = service.current_page
    try {
        dispatch(updateService({ id, status,  cpage}));
  ...
  }, [searchResults, service?.data, searchQuery]);
  ...
  const handleSave = async (e, id) => {
    ...
    try {
      ...
      const resultAction = await dispatch(storePrice({ formData, id }));
  
      // Check if the action was fulfilled successfully
      if (storePrice.fulfilled.match(resultAction)) {
   ...
   <>
        ...
        <table className="w-full table-auto">
          ...
          {dataToDisplay?.map((val,idx)=>(
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">{val.service?.title} </p>
              </td>
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">{val.name} </p>
              </td>
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">{val.email} </p>
              </td>
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
                  ...
                  <Link className="hover:text-primary" to = {`view_service_request/${val.id}`} >
                    <svg
                  ...
                  {val?.status === 'Encore' && isLoading ? (
                        ...
                        val?.status === 'Traité' ? (
                          <p className="text-success dark:text-white text-center">
                            {val?.status}
                          </p>
                        ) : (
                  ...
                  <div>
                      {val.price? 
                        ...
                        />
                        : 
                        <button onClick={() => handleInputChange(val.id, true)} className='border px-4 py-1 border-meta-8 text-meta-8 hover:bg-blite'>Prix</button>
                        }
                  </div>
    ...
    <div className='mt-10 flex flex-row justify-between'>
      {service.next_page_url && (
        ...
        onClick={() => handlePagination(service.next_page_url)}>
          Next <i class="fa-solid fa-chevron-right"></i>
        </button>
      )}
      {service.prev_page_url && (
        <button className='text-black hover:bg-bodydark1 transition duration-700 dark:text-white px-6 py-3 bg-white rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark' 
        onClick={() => handlePagination(service.prev_page_url)}>
