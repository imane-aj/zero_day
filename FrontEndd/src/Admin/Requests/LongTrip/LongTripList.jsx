...
import { LongTrip, storePriceTrip, updateLongTrip } from '../../../Redux/RequestSlice';

const LongTripList = ({dispatch,searchQuery,}) => {
  const trip = useSelector((state) => state.req.data)
  ...
  useEffect(() => {
    dispatch(LongTrip(`${url}trip_request?page=1`));
  }, [dispatch]);

  const handlePagination = (url) => {
    console.log('Pagination URL:', url);
    dispatch(LongTrip(url));
  };

  const handleStatusChange = (e ,id) => {
    e.preventDefault();
    const status = 'Traité';
    const cpage = trip.current_page
    try {
        dispatch(updateLongTrip({ id, status,  cpage}));
    } catch (error) {
  ...
  useEffect(() => {
    if (searchResults.data && searchQuery) {
      ...
      setDataToDisplay(trip?.data);
    }
  }, [searchResults, trip?.data, searchQuery]);
  ...
  const handleSave = async (e, id) => {
    ...
    try {
      ...
      const resultAction = await dispatch(storePriceTrip({ formData, id }));
  
      // Check if the action was fulfilled successfully
      if (storePriceTrip.fulfilled.match(resultAction)) {
   ...
   <>
        ...
        <table className="w-full table-auto">
          ...
          {dataToDisplay?.map((val,idx)=>(
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">{val.long_trip?.title} </p>
              </td>
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
                  ...
                  <Link className="hover:text-primary" to = {`view_lonTrip_request/${val.id}`} >
                    <svg
    ...
    <div className='mt-10 flex flex-row justify-between'>
      {trip.next_page_url && (
        ...
        onClick={() => handlePagination(trip.next_page_url)}>
          Next <i class="fa-solid fa-chevron-right"></i>
        </button>
      )}
      {trip.prev_page_url && (
        <button className='text-black hover:bg-bodydark1 transition duration-700 dark:text-white px-6 py-3 bg-white rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark' 
        onClick={() => handlePagination(trip.prev_page_url)}>
