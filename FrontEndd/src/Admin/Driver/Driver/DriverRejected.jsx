...
import { DriversR, Search_driver } from '../../../Redux/DriverSlice';
const DriverRejected = () => {
  ...
  const driver = useSelector((state) => state.driver.dataR)
  ...
  useEffect(() => {
    dispatch(DriversR(`${url}drivers_refuse?page=1`));
  }, [dispatch]);

  const handlePagination = (url) => {
    console.log('Pagination URL:', url);
    dispatch(DriversR(url));
  };
      ...
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          {dataToDisplay?.map((val,idx)=>(
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <p className="text-black dark:text-white m-auto">{val.commit ? val.commit : "-"} </p>
              </td>
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
                  ...
                  <p className='text-danger'>{val?.status}</p>
