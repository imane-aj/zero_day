...
import { ShowPost, storePost, updatePost } from '../../redux/BlogSlice';
...
export default function NewPost() {
    ...
    console.log(error)
    const post = useSelector((state)=>state.blog.article)
    console.log(title)
    ...
    useEffect(() => {
        if (id) {
          dispatch(ShowPost(id)); // Dispatch the action to fetch job data
          if (post) {
            console.log('post',post);
            setArticle(post?.data?.article);
            setTitle(post?.data?.title);
            setImg(post?.data?.img);
        }
        }
    }, [dispatch, id, post]);
    ...
    const handleSave = (e)=>{
        ...
        if(id){
            dispatch(updatePost({id,formData})).then((res) => {
                if (res.type === 'blog/updatePost/fulfilled') {
                  navigate('/admin/blog')
                  Swal.fire('Success', 'This post was updated succefuly', 'success');
                }
              });
            }else{
                dispatch(storePost(formData)).then((res) => {
                    if (res.type === 'blog/storePost/fulfilled') {
                      navigate('/admin/blog')
                      Swal.fire('Success', 'This post was added succefuly', 'success');
                    }
                });
        };
    }
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <form enctype='multipart/form-data'>
            ...
            <div>
                ...
                <ReactQuill value={article} onChange={handleQuillChange} style={{height: '150px', marginBottom:'4em'}} />
                {error && error.errors && error.errors.article && <span className='text-danger mb-10'>{error && error?.errors?.article[0]}</span>}
            </div>
