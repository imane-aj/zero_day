...
import { deletetransfer, transfers } from '../../Redux/TransferSlice';

const TransferList = ({dispatch,searchQuery,}) => {
  const transfer = useSelector((state) => state.transfer.data)
  console.log('val',TransferList)
  const searchResults = useSelector((state) => state.transfer.search)
  const [dataToDisplay, setDataToDisplay] = useState([]); // Initially set to all data
 
  useEffect(() => {
    dispatch(transfers(`${url}transfers?page=1`));
  }, [dispatch]);

  const handlePagination = (url) => {
    console.log('Pagination URL:', url);
    dispatch(transfers(url));
  };
    ...
    console.log(id)
      Swal.fire({
        ...
        if (result.isConfirmed) {
          dispatch(deletetransfer(id)).then((res) => {
            console.log('res.data', res)
            if (res.type === 'transfer/deletetransfer/fulfilled') {
  ...
  useEffect(() => {
    if (searchResults.data && searchQuery) {
      ...
      setDataToDisplay(transfer?.data);
    }
  }, [searchResults, transfer?.data, searchQuery]);
  return (
   <>
          ...
          {dataToDisplay?.map((val,idx)=>(
              ...
              <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                <h5 className="font-medium text-black dark:text-white">
                  <img src={val.cover_url} className='w-[80px] h-12 text-start'/>
                </h5>
              </td>
              ...
              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
                  ...
                  <Link className="hover:text-primary" to = {`edit_transfer/${val.id}`}>
    ...
    <div className='mt-10 flex flex-row justify-between'>
      {transfer?.next_page_url && (
        ...
        onClick={() => handlePagination(transfer?.next_page_url)}>
          Next <i class="fa-solid fa-chevron-right"></i>
        </button>
      )}
      {transfer?.prev_page_url && (
        <button className='text-black hover:bg-bodydark1 transition duration-700 dark:text-white px-6 py-3 bg-white rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark' 
        onClick={() => handlePagination(transfer?.prev_page_url)}>
